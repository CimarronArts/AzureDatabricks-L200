...
#Creating paths and loading data files
...
# as creates a new column tranformation
...
#concatenate store_id and id to create a unique key
silver_df = spark.sql(f"""
...
#now we are getting our transaction fact tables
...
# MAGIC %sql
# MAGIC -- Run this to see newly processed rows from 2 new files
# MAGIC select file_path, count(*) number_of_records
# MAGIC from bronze_sales
# MAGIC group by file_path
...
# MAGIC %fs head /FileStore/scott_smith/deltademoasset/tables/silver/silver_sale_items/_delta_log/00000000000000000001.json
...
# MAGIC %sql -- We have not moved the data from bronze to silver
# MAGIC 
# MAGIC select * from silver_sales
